...
WaypointQueue Tower::get_instructions(Aircraft& aircraft)
{
    ...
    else
    {
        ...
        if (!terminal.is_servicing())
        {
            ...
            aircraft.is_already_serviced = true;
...
{
    assert(&aircraft);
    const auto iter = reserved_terminals.find(&aircraft);
    if(iter != reserved_terminals.end()){
        const auto terminal_num = iter->second;
        Terminal& terminal = airport.get_terminal(terminal_num);
        terminal.del_aircraft();
        reserved_terminals.erase(iter);
    }
}